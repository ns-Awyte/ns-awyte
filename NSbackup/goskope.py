...
class Auth:
	...
	### Login for tenants with version < R97
	def loginV1(self):
		print("called loginV1")

		data = {'username':self.username, 'password':self.password, 'token':self.token }

		self.request = self.session.post(self.baseurl + "/login/authenticate", data=data, allow_redirects=False)

		self.gettoken()

		return self.request

	### Legacy first time password change for tenants < R97
	def firstpwdchange(self, new_password):
		print("called firstpwdchange")

		data = {'first_login_hidden':'yes', 'first_login_password':new_password, 
                        'confirm_password':new_password, 'windowName':'', 'redirect':'', 
                        'token':self.token }

		self.request = self.session.post(self.baseurl + "/login/firstPwdChange", data=data)

		return self.request

	### Legacy terms of service screen for tenants < R97
	def settosseen(self):
		print("called settosseen")

		data = {'cb_agree':'agree', 'windowName':'', 'redirect':'', 'token':self.token }

		self.request = self.session.post(self.baseurl + "/login/setTOSSeen", data=data)

		return self.request

	### Login for tenants with version >= R97
	def loginV2(self):
		print("called loginV2")
		...
		if self.request.status_code == 200 and (errorCode == 'first_login' or re.search("/login/firstPwdChange",self.request.text)):
	...
	### Attempts to guess the tenant version by examining HTML return from login page
	def getVersion(self):
		print("called getVersion")
		self.request = self.session.get(self.baseurl + "/locallogin")
		match        = re.search('css\?v\=([\d\.]+)',self.request.text,re.IGNORECASE)
		version      = match.group(1)

		return version

	### Returns dict of user status
	def isAccessAllowed(self):
		print("called isAccessAllowed")

		if self.version < "97.0.0.0":
			return None

		self.request = self.session.get(self.baseurl + "/login/isAccessAllowed", headers=self.headers)
		data         = json.loads(self.request.text)['data']
		return data
